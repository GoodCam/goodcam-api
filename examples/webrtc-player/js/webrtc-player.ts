...
class StreamingSession {
  ...
  playButtonElem: HTMLButtonElement
  bufferLocalICECandidates: boolean
  bufferedLocalICECandidates: any[]
  bufferRemoteICECandidates: boolean
  bufferedRemoteICECandidates: any[]
   ...
   *
   * @param {Element} videoElem video element that will display the video content
   ...
   */
  constructor(videoElem: HTMLVideoElement, playButtonElem: HTMLButtonElement, errorCallback: (err: any) => void) {
   ...
   */
  async connect(url: string, stunServers: string[]) {
    ...
    this.connection.oniceconnectionstatechange = async (e: any) => {
      ...
      if (e.target?.iceConnectionState === 'failed' || e.target?.iceConnectionState === 'disconnected') {
        this.terminate(new Error(`ICE connection state is ${e.target?.iceConnectionState}`))
      }
    }
   ...
   */
  sendLocalCandidate(candidate: RTCIceCandidate | null) {
   ...
   */
  processWsMessage(msg: MessageEvent) {
    if (msg.type === 'message') {
  ...
  async processOffer(offer: any) {
    try {
      ...
      for (const candidate of this.bufferedRemoteICECandidates) {
      ...
      for (const candidate of this.bufferedLocalICECandidates) {
   ...
   */
  async processRemoteCandidate(candidate: RTCIceCandidateInit) {
 ...
 * @returns candidate
 */
function candidateFromMessage(msg: any) {
  if (!msg) {
    return null
  }
 ...
 */
function candidateToMessage(candidate: RTCIceCandidate | null) {
  if (!candidate) {
    return null
  }
...
export class VideoPlayer {
   ...
   * @param {Array} stunServers STUN servers
   */
  play(url: string, stunServers: string[]) {
